...
double get_point(xc_func_type_cuda *func, double point[5], double *e, double der[5], int which)
{
  switch(func->info->family)
    {
    case XC_FAMILY_LDA:
      xc_lda_exc_vxc_cuda(func, 1, &(point[0]), e, &(der[0]));
      break;
    case XC_FAMILY_GGA:
...
void get_vxc(xc_func_type_cuda *func, double point[5], double *e, double der[5])
{
  get_point(func, point, e, der, 0);
}

void get_fxc(xc_func_type_cuda *func, double point[5], double der[5][5])
{
  ...
  switch(func->info->family)
    {
    case XC_FAMILY_LDA:
      xc_lda_fxc_cuda(func, 1, &(point[0]), v2rho);
      break;
    case XC_FAMILY_GGA:
    case XC_FAMILY_HYB_GGA:
...
void first_derivative(xc_func_type_cuda *func, double point[5], double der[5], int which)
{
  int i;
...
void second_derivatives(xc_func_type_cuda *func, double point[5], double der[5][5])
{
  int i;
...
void print_error(char *type, char *what, double diff, xc_func_type_cuda *func, double *p)
{
...
void test_functional(int functional)
{
  ...
  const xc_func_info_type_cuda *info;
  int i, j, k, p_max[6][5];
  ...
  if(xc_func_init_cuda(&func, functional, nspin) != 0){
  ...
  if(functional == XC_LDA_C_2D_PRM){
    ...
    xc_func_set_ext_params_cuda(&func, lda_c_2d_prm_params);
  }  
