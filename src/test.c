...
void test_lda()
{
  xc_func_type_cuda l1, l2, l3;
  int i;
  
  //xc_func_init_cuda(&l1, XC_LDA_XC_KSDT2, XC_POLARIZED);
  //xc_func_init_cuda(&l1, XC_LDA_C_PW, XC_UNPOLARIZED);
  xc_func_init_cuda(&l3, XC_LDA_X, XC_UNPOLARIZED);
  ...
  for(i=0; i<=1000; i++){
    ...
    xc_lda_cuda(&l1, 1, rho, &ec1, vc1, NULL, NULL);
    //xc_lda_cuda(&l2, 1, rho, &ec2, vc2, NULL, NULL);
    //xc_lda_fxc_fd(&l2, rho, fxc2);
    //xc_lda_kxc_fd(&l2, rho, kxc2);

    //rho[0] = dens; rho[1] = 0.0;
    //xc_lda_cuda(&l3, rho, &ec3, vc3, fxc3, kxc3);
...
void test_ak13()
{
  xc_func_type_cuda gga;
  double beta = 0.13;
  double x, rho[2] = {0.0, 0.0}, sigma[3] = {0.0, 0.0, 0.0}, zk, vrho[2], vsigma[3];
  double tmp1, tmp2;
  int i;

  xc_func_init_cuda(&gga,  XC_GGA_X_AK13,  XC_POLARIZED);
  ...
  for(i=0; i<=10000; i++){
    ...
    xc_gga_exc_vxc_cuda(&gga,  1, rho, sigma, &zk, vrho, vsigma);
...
void test_enhance()
{
  ...
  xc_func_type_cuda gga1, gga2, gga3, gga4;

  xc_func_init_cuda(&gga1,  XC_GGA_X_B88, XC_POLARIZED);
  xc_func_init_cuda(&gga2, XC_GGA_X_GG99, XC_POLARIZED);
...
void test_gga()
{
  ...
  xc_func_init_cuda(&gga,  XC_GGA_C_PW91,  XC_POLARIZED);
...
void test_mgga()
{
  xc_func_type_cuda mgga1, mgga2;
  int i;

  xc_func_init_cuda(&mgga1, XC_GGA_C_LYP, XC_POLARIZED);
  xc_func_init_cuda(&mgga2, XC_MGGA_X_SCAN, XC_POLARIZED);
  ...
  for(i=0; i<=1000; i++){
    ...
    //xc_mgga_cuda(&mgga1, 1, rho, sigma, lapl, tau, 
    //	     &zk,  vrho, vsigma, vlapl, vtau, 
    //	     v2rho2, v2sigma2, v2lapl2, v2tau2, v2rhosigma, v2rholapl, v2rhotau,
    //	     v2sigmalapl, v2sigmatau, v2lapltau);
    //xc_mgga_cuda(&mgga2, 1, rho, sigma, lapl, tau, 
    //	     &zk2,  vrho2, vsigma2, vlapl2, vtau2, 
    // 	     NULL, v2sigma2, v2lapl2, v2tau2, v2rhosigma, v2rholapl, v2rhotau,
    //	     v2sigmalapl, v2sigmatau, v2lapltau);
    //xc_mgga_exc_cuda(&mgga2, 1, rho, sigma, lapl, tau, 
    //		 &zk2);
    xc_gga_exc_vxc_cuda(&mgga1, 1, rho, sigma, 
		     &zk,  vrho, vsigma);
    xc_mgga_exc_vxc_cuda(&mgga2, 1, rho, sigma, lapl, tau, 
		     &zk2,  vrho2, vsigma2, vlapl2, vtau2);

    fprintf(stderr, "%16.10lf\t%16.10lf\t%16.10lf\n", sigma[2], zk2*(rho[0]+rho[1]), vsigma2[2]);
  }

  xc_func_end_cuda(&mgga1);
  xc_func_end_cuda(&mgga2);
}
...
void test_neg_rho()
{
  ...
  for(func_id=1; func_id<1000; func_id++){
    if(xc_func_init_cuda(&func, func_id, XC_POLARIZED) != 0) continue;
    ...
    case XC_FAMILY_LDA:
      xc_lda_exc_vxc_cuda(&func, 5, &rho[0][0], zk, &vk[0][0]);
      break;
    case XC_FAMILY_GGA:
      xc_gga_exc_vxc_cuda(&func, 5, &rho[0][0], &sigma[0][0], zk, &vk[0][0], &vsigma[0][0]);
      break;
    }
