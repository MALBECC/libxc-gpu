...
static void
gga_x_ityh_init(xc_func_type_cuda *p)
{
...
void 
xc_gga_x_ityh_set_params(xc_func_type_cuda *p, int func_id, double omega)
{
  ...
  if(func_id != -1 && params->func_id == -1){ /* intialize stuff */
    p->n_func_aux  = 1;
    p->func_aux    = (xc_func_type_cuda **) malloc(sizeof(xc_func_type_cuda *));
    p->func_aux[0] = (xc_func_type_cuda  *) malloc(sizeof(xc_func_type_cuda  ));
  }

  if(func_id != -1 && params->func_id != func_id){
    if(params->func_id != -1)
      xc_func_end_cuda (p->func_aux[0]);

    params->func_id = func_id;
    xc_func_init_cuda (p->func_aux[0], params->func_id, p->nspin);
...
static inline void 
func_3(const xc_func_type_cuda *p, int order, double x, double ds,
...
static inline void 
func(const xc_func_type_cuda *p, xc_gga_work_c_t *r)
{
  int i;
...
const xc_func_info_type_cuda xc_func_info_gga_x_ityh = {
  XC_GGA_X_ITYH,
  XC_EXCHANGE,
