...
void 
xc_mix_init(xc_func_type_cuda *p, int n_funcs, const int *funcs_id, const double *mix_coef)
{
  int ii;
  ...
  p->func_aux   = (xc_func_type_cuda **) malloc(n_funcs*sizeof(xc_func_type_cuda *));

  for(ii=0; ii<n_funcs; ii++){
    p->mix_coef[ii] = mix_coef[ii];
    p->func_aux[ii] = (xc_func_type_cuda *) malloc(sizeof(xc_func_type_cuda));
    xc_func_init_cuda (p->func_aux[ii], funcs_id[ii], p->nspin);
  }
