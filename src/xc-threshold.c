...
/*----------------------------------------------------------*/
void print_header(xc_func_type_cuda *func)
{
...
/*----------------------------------------------------------*/
void print_values(xc_values_type *values, xc_func_type_cuda *func){
...
int main(int argc, char *argv[])
{
  ...
  if (xc_func_init_cuda(&func, id, nspin) != 0) {
    fprintf(stderr, "Functional '%d' not found\n", id);
    exit(1);
  }
  default_threshold = func.dens_threshold;
  xc_func_set_dens_threshold_cuda(&func, MIN_DENS/10.0);
  if(func.info->flags & XC_FLAGS_HAVE_EXC){
    pzk = &values.zk;
  }
  if(func.info->flags & XC_FLAGS_HAVE_VXC){
  ...
  if (testcase > 3) xc_func_init_cuda(&tf, XC_LDA_K_TF, nspin);
  ...
  do {
    ...
    if (testcase > 3) {
      xc_lda_exc_cuda(&tf, 1, &rho1a, &tau1a);
      xc_lda_exc_cuda(&tf, 1, &rho2a, &tau2a);
    }
    ...
    if (testcase == 5) {
      xc_lda_exc_cuda(&tf, 1, &rho1b, &tau1b);
      xc_lda_exc_cuda(&tf, 1, &rho2b, &tau2b);
    }
    ...
    case XC_FAMILY_LDA:
      xc_lda_cuda(&func, 1, values.rho, pzk, pvrho, pv2rho2, pv3rho3);
      break;
    case XC_FAMILY_GGA:
    case XC_FAMILY_HYB_GGA:
      xc_gga_cuda(&func, 1, values.rho, values.sigma,
	     pzk, pvrho, pvsigma, pv2rho2, pv2rhosigma, pv2sigma2, NULL, NULL, NULL, NULL);
      break;

    case XC_FAMILY_MGGA:
    case XC_FAMILY_HYB_MGGA:
      xc_mgga_cuda(&func, 1, values.rho, values.sigma, values.lapl, values.tau,
  ...
  if (testcase > 3) xc_func_end_cuda(&tf);
