...
#define XC_LDA_C_1D_CSC         1018 /* Casula, Sorella, and Senatore 1D correlation     */
...
static void lda_c_1d_csc_init(xc_func_type_cuda *p)
{
  //printf("lda_c_1d_csc_init(...)\n");
  assert (p != NULL && p->params == NULL);
  p->params = malloc(sizeof(lda_c_1d_csc_params));
  p->size_of_params_gpu = sizeof(lda_c_1d_csc_params);

  // Alloc memory for params_gpu in the device.
  checkCudaErrors(cudaMalloc((void**)(&(p->params_gpu)), p->size_of_params_gpu));

}

static void lda_c_1d_csc_end(xc_func_type_cuda *p)
{
    //printf("lda_c_1d_csc_end(...)\n");

    assert(p != NULL && p->params != NULL);
    lda_c_1d_csc_params *params = (lda_c_1d_csc_params *)(p->params);

    if (params->para_gpu != NULL) {
	checkCudaErrors(cudaFree(params->para_gpu));
    }
    if (params->ferro_gpu != NULL) {
	checkCudaErrors(cudaFree(params->ferro_gpu));
    }
...
#include "maple2c/lda_c_1d_csc.cu"
...
static void set_ext_params(xc_func_type_cuda *p, const double *ext_params)
{
  //printf("set_ext_params(...)\n");
  ...
  if(params->para == NULL){
    fprintf(stderr, "Invalid value of parameters (inter,b) = (%d,%f) in lda_c_1d_csc_set_params", params->interaction, params->bb);
    exit(1);
  }

  // Now set the data for CUDA
  checkCudaErrors(cudaMemcpy(p->params_gpu, p->params, p->size_of_params_gpu, cudaMemcpyHostToDevice));

  // Alloc memory for para_gpu and ferro gpu in the device.
  checkCudaErrors(cudaMalloc((void**)(&(params->para_gpu)),  10*sizeof(double)));
  checkCudaErrors(cudaMalloc((void**)(&(params->ferro_gpu)), 10*sizeof(double)));

  checkCudaErrors(cudaMemcpy(params->para_gpu,  params->para,  10*sizeof(double), cudaMemcpyHostToDevice));
  checkCudaErrors(cudaMemcpy(params->ferro_gpu, params->ferro, 10*sizeof(double), cudaMemcpyHostToDevice));

}


extern "C" const xc_func_info_type_cuda xc_func_info_lda_c_1d_csc = {
  XC_LDA_C_1D_CSC,
  XC_CORRELATION,
  ...
  lda_c_1d_csc_init, lda_c_1d_csc_end,
  work_lda, NULL, NULL
};
