...
static void
gga_lb_init(xc_func_type_cuda *p)
{
  ...
  p->n_func_aux  = 1;
  p->func_aux    = (xc_func_type_cuda **) malloc(1*sizeof(xc_func_type_cuda *));
  p->func_aux[0] = (xc_func_type_cuda *)  malloc(  sizeof(xc_func_type_cuda));

  xc_func_init_cuda(p->func_aux[0], XC_LDA_X, p->nspin);
...
void 
xc_gga_lb_modified_cuda(const xc_func_type_cuda *func, int np, const double *rho, const double *sigma, double r, double *vrho)
{
  int ip, is, is2;
  ...
  xc_lda_vxc_cuda(func->func_aux[0], np, rho, vrho);
...
static void 
gga_x_lb(const xc_func_type_cuda *p, int np, const double *rho, const double *sigma,
	 double *zk, double *vrho, double *vsigma,
	 double *v2rho2, double *v2rhosigma, double *v2sigma2,
	 double *v3rho3, double *v3rho2sigma, double *v3rhosigma2, double *v3sigma3)
{
  if (vrho != NULL)
    xc_gga_lb_modified_cuda(p, np, rho, sigma, 0.0, vrho);
}
...
const xc_func_info_type_cuda xc_func_info_gga_x_lb = {
  XC_GGA_X_LB,
  XC_EXCHANGE,
...
const xc_func_info_type_cuda xc_func_info_gga_x_lbm = {
  XC_GGA_X_LBM,
  XC_EXCHANGE,
