...
extern xc_func_info_type_cuda 
  *xc_lda_known_funct[], 
  *xc_gga_known_funct_cuda[],
...
int xc_family_from_id_cuda(int id, int *family, int *number)
{
  ...
  /* or is it a GGA? */
  for(ii=0; xc_gga_known_funct_cuda[ii]!=NULL; ii++){
    if(xc_gga_known_funct_cuda[ii]->number == id){
...
xc_func_type_cuda *xc_func_alloc_cuda()
{
  ...
  func = (xc_func_type_cuda *) malloc (sizeof (xc_func_type_cuda));
  return func;
}

/*------------------------------------------------------*/
int xc_func_init_cuda(xc_func_type_cuda *func, int functional, int nspin)
{
  int number;

  //printf("xc_func_init_cuda\n");
  assert(func != NULL);
  ...
  switch(xc_family_from_id_cuda(functional, NULL, &number)){
  case(XC_FAMILY_LDA):
    func->info = xc_lda_known_funct[number];
    break;

  case(XC_FAMILY_GGA):
    func->info = xc_gga_known_funct_cuda[number];
    break;
...
/*------------------------------------------------------*/
void xc_func_end_cuda(xc_func_type_cuda *func)
{
  ...
  if(func->n_func_aux > 0){
    ...
    for(ii=0; ii<func->n_func_aux; ii++){
      xc_func_end_cuda(func->func_aux[ii]);
...
void xc_func_set_dens_threshold_cuda(xc_func_type_cuda *p, double dens_threshold)
{
  ...
  for(ii=0; ii<p->n_func_aux; ii++) {
    xc_func_set_dens_threshold_cuda(p->func_aux[ii], dens_threshold);
  }
}
